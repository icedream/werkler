...
// Used by the fallback implementation when AST parsing fails.
var shellSplitRe = regexp.MustCompile(`[;|&\n]+`)

// extractShellSubCommands parses the shell script using mvdan.cc/sh/v3/syntax,
// walks the AST, and returns the resolved absolute paths of each distinct
// executable found. Falls back to the old regex-based approach if parsing fails.
func extractShellSubCommands(script, cwd string) []string {
	// Try AST-based parsing first.
	parser := syntax.NewParser()
	node, err := parser.Parse(strings.NewReader(script), "")
	if err != nil {
		return extractShellSubCommandsFallback(script, cwd)
	}
	...
	syntax.Walk(node, func(n syntax.Node) bool {
		// CallExpr is the node that represents a concrete command invocation
		// (e.g. "echo foo", "grep bar").  Collect its first word as the
		// executable name.
		...
		cmdName := wordToString(cmd.Args[0])
		// Skip shell keywords and builtins.
		if shellKeywords[cmdName] || containsShellVar(cmdName) {
			return true
		}
...
// extractShellSubCommandsFallback is the old regex-based extraction, used as a
// fallback when mvdan.cc/sh/v3 parsing fails.
func extractShellSubCommandsFallback(script, cwd string) []string {
	var cmds []string
	seen := make(map[string]bool)
...
// wordToString converts a syntax.Word to its source representation using
// mvdan.cc/sh/v3/syntax's printer. This gives us the exact text as it
// appeared in the script (with quotes preserved).
...
// shellKeywords is the set of tokens that are shell control-flow keywords or
// non-executable constructs; they are not independent executables and should
// not be added to the path approval list.
